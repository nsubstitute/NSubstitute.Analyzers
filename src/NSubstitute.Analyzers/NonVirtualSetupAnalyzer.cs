...
namespace NSubstitute.Analyzers
{
    ...
    public class NonVirtualSetupAnalyzer : AbstractNonVirtualSetupAnalyzer<SyntaxKind, MemberAccessExpressionSyntax, InvocationExpressionSyntax>
    {
        ...
        protected override bool? CanBeSetuped(SyntaxNode accessedMember, SymbolInfo symbolInfo)
        {
            if (accessedMember is LiteralExpressionSyntax)
            {
                return false;
            }

            return base.CanBeSetuped(accessedMember, symbolInfo);
        }
    }
}