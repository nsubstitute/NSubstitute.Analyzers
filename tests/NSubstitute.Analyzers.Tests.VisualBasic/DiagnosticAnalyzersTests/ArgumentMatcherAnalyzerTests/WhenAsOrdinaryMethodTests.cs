...
{
    [CombinatoryData(
        "SubstituteExtensions.When",
        "SubstituteExtensions.When(Of Foo)",
        "SubstituteExtensions.WhenForAnyArgs",
        "SubstituteExtensions.WhenForAnyArgs(Of Foo)")]
...
Namespace MyNamespace
    ...
    Public Class FooTests
        Public Sub Test()
            ...
            {method}(substitute, Function(ByVal x As Foo)
                x.Bar({arg})
            End Function).[Do](Function(x)
                Throw New NullReferenceException()
            End Function)
            {method}(substitute, AddressOf SubstituteCall).[Do](Function(x)
                Throw New NullReferenceException()
            End Function)
        End Sub
    ...
    Public Class FooTests
        Public Sub Test()
            ...
            {method}(substitute, Function(ByVal x As Foo)
                x.Bar({arg})
            End Function, 1)
            {method}(substitute, Function(x) x.Bar({arg}), 1)
        End Sub
