...
namespace MyNamespace
{
    ...
    public class FooTests
    {
        public void Test()
        {
            var substitute = NSubstitute.Substitute.For<Foo>(1, 2);
            var otherSubstitute = NSubstitute.Substitute.For<Foo>(constructorArguments: new [] { 1, 2 });
            var yetAnotherSubstitute = NSubstitute.Substitute.For<Foo>(new [] { 1, 2 });
        }
    }
}";
            await VerifyNoDiagnostic(source);
