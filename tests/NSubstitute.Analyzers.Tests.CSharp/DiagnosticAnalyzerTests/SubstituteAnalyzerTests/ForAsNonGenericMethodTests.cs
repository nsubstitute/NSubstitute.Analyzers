...
namespace MyNamespace
{
    ...
    public class FooTests
    {
        public void Test()
        {
            var substitute = [|NSubstitute.Substitute.For(new [] { typeof(Foo) }, new object[] { 1, 2 })|];
            var otherSubstitute = [|NSubstitute.Substitute.For(typesToProxy: new [] { typeof(Foo) }, constructorArguments: new object[] { 1, 2 })|];
            var yetAnotherSubstitute = [|NSubstitute.Substitute.For(constructorArguments: new object[] { 1, 2 }, typesToProxy: new [] { typeof(Foo) })|];
        }
    }
}";
            await VerifyNoDiagnostic(source);
