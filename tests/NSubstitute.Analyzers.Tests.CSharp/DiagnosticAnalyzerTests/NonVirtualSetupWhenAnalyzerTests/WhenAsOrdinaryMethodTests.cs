...
namespace NSubstitute.Analyzers.Tests.CSharp.DiagnosticAnalyzerTests.NonVirtualSetupWhenAnalyzerTests
{
    ...
    {
        [Theory]
        [InlineData("sub => sub.Bar()", 58)]
        [InlineData("delegate(Foo sub) { sub.Bar(); }", 71)]
        [InlineData("sub => { sub.Bar(); }", 60)]
...
{{
    ...
    {{
        ...
        [Theory]
        [InlineData("sub => sub.Bar()", 58)]
        [InlineData("delegate(Foo2 sub) { sub.Bar(); }", 72)]
        [InlineData("sub => { sub.Bar(); }", 60)]
...
{{
    ...
    {{
        ...
        [Theory]
        [InlineData("sub => { var x = sub.Bar; }", 68)]
        [InlineData("delegate(Foo sub) { var x = sub.Bar; }", 79)]
...
{{
    ...
    {{
        ...
        [Theory]
        [InlineData("sub => { var x = sub[1]; }", 68)]
        [InlineData("delegate(Foo sub) { var x = sub[1]; }", 79)]
