...
namespace NSubstitute.Analyzers.Tests.CSharp.DiagnosticAnalyzerTests.NonVirtualSetupWhenAnalyzerTests
{
    ...
    {
        [Theory]
        [InlineData("sub => sub.Bar()", 51)]
        [InlineData("delegate(Foo sub) { sub.Bar(); }", 64)]
        [InlineData("sub => { sub.Bar(); }", 53)]
...
{{
    ...
    {{
        ...
        [Theory]
        [InlineData("sub => sub.Bar()", 52)]
        [InlineData("delegate(Foo2 sub) { sub.Bar(); }", 66)]
        [InlineData("sub => { sub.Bar(); }", 54)]
...
{{
    ...
    {{
        ...
        [Theory]
        [InlineData("sub => { var x = sub.Bar; }", 61)]
        [InlineData("delegate(Foo sub) { var x = sub.Bar; }", 72)]
...
{{
    ...
    {{
        ...
        [Theory]
        [InlineData("sub => { var x = sub[1]; }", 61)]
        [InlineData("delegate(Foo sub) { var x = sub[1]; }", 72)]
