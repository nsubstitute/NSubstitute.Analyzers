...
namespace NSubstitute.Analyzers.Tests.CSharp.DiagnosticAnalyzerTests.NonVirtualSetupWhenAnalyzerTests
{
    ...
    {
        [Theory]
        [InlineData("sub => sub.Bar()", 19, 51)]
        [InlineData("delegate(Foo sub) { sub.Bar(); }", 19, 64)]
        [InlineData("sub => { sub.Bar(); }",  19, 53)]
...
{{
    ...
    {{
        ...
        [Theory]
        [InlineData("sub => sub.Bar()", 24, 52)]
        [InlineData("delegate(Foo2 sub) { sub.Bar(); }", 24, 66)]
        [InlineData("sub => { sub.Bar(); }", 24, 54)]
...
{{
    ...
    {{
        ...
        [Theory]
        [InlineData("sub => { var x = sub.Bar; }", 16, 61)]
        [InlineData("sub => { int x; x = sub.Bar; }", 16, 64)]
        [InlineData("delegate(Foo sub) { var x = sub.Bar; }", 16, 72)]
...
{{
    ...
    {{
        ...
        [Theory]
        [InlineData("sub => { var x = sub[1]; }", 16, 61)]
        [InlineData("delegate(Foo sub) { var x = sub[1]; }", 16, 72)]
