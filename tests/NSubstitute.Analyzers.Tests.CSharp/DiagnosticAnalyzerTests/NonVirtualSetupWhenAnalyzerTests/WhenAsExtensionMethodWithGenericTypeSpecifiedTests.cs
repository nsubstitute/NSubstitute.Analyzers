...
{
    public class WhenAsExtensionMethodWithGenericTypeSpecifiedTests : NonVirtualSetupWhenDiagnosticVerifier
    {
        [Theory]
        [InlineData("sub => sub.Bar()", 19, 41)]
        [InlineData("delegate(Foo sub) { sub.Bar(); }", 19, 54)]
        [InlineData("sub => { sub.Bar(); }", 19, 43)]
...
{{
    ...
    {{
        ...
        {{
            ...
            substitute.When<Foo>({whenAction}).Do(callInfo => i++);
        }}
    }}
}}";
...
{{
    ...
    {{
        ...
        {{
            ...
            substitute.When<Foo2>({whenAction}).Do(callInfo => i++);
        }}
    }}
}}";
            await VerifyDiagnostic(source);
...
{{
    ...
    {{
        ...
        {{
            ...
            substitute.When<Func<int>>({whenAction}).Do(callInfo => i++);
        }}
    }}
}}";
            await VerifyDiagnostic(source);
        ...
        [Theory]
        [InlineData("sub => sub.Bar()", 24, 42)]
        [InlineData("delegate(Foo2 sub) { sub.Bar(); }", 24, 56)]
        [InlineData("sub => { sub.Bar(); }", 24, 44)]
...
{{
    ...
    {{
        ...
        {{
            ...
            substitute.When<IFoo>({whenAction}).Do(callInfo => i++);
        }}
    }}
}}";
            await VerifyDiagnostic(source);
   ...
   public interface IFoo<T>
    ...
    {{
        ...
        {{
            ...
            substitute.When<IFoo<int>>({whenAction}).Do(callInfo => i++);
        }}
    }}
}}";
            await VerifyDiagnostic(source);
...
{{
    ...
    {{
        ...
        {{
            Foo substitute = null;
            substitute.When<Foo>({whenAction}, 1);
        }}
    }}
}}";
            await VerifyDiagnostic(source);
        ...
        [Theory]
        [InlineData("sub => { var x = sub.Bar; }", 16, 51)]
        [InlineData("sub => { int x; x = sub.Bar; }", 16, 54)]
        [InlineData("delegate(Foo sub) { var x = sub.Bar; }", 16, 62)]
...
{{
    ...
    {{
        ...
        [Theory]
        [InlineData("sub => { var x = sub[1]; }", 16, 51)]
        [InlineData("delegate(Foo sub) { var x = sub[1]; }", 16, 62)]
...
namespace MyNamespace
{
    ...
    public class FooTests
    {
        public void Test()
        {
            ...
            substitute.When<Foo>(SubstituteCall).Do(callInfo => i++);
            substitute.Bar();
        }
    }
}";
